...
for i = 1:length(ell)
    ...
    gamma = 1/tau(index15);
    nsep15 = getDCSNoise(200e3,T,2,Beta,gamma,tau); %50 hz.
    ...
    for detector = Detector
        ...
        g2_25(detector,:) = Beta.*normsep25.*normsep25 + noise25 + 1;
        %g2_25 = Beta.*normsep25.*normsep25  + 1;
        %cuttoff
    end
    meang2_25 = mean(g2_25);
    foo25 = min(find(meang2_25 <= cutoff))+ good_start -1;
    if isempty(foo25) || foo25 < good_start, foo25=70;, end%Fit first 70 points
    meang2_25 = meang2_25(1:foo);
    tau25 = tau(1:foo);
    
    dbbeta25(i,:) = fminsearchbnd(@(x) twolayercost(x,Beta,meang2_25,n,Reff,mua1,mus1,calcdb1,tau25,lambda,rho2,w,mua2,mus2),guessc,lbc,ubc);
