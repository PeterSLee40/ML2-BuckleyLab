...
%   $Revision: 1.1.6.3 $  $Date: 2007/06/14 05:19:17 $
...
if (nsgrows >= max(1, .01*size(Acurr,1)))
    ...
    if computeLambda
        ...
        if length(sgrows) ~= length(sgcols)
            error('optim:simplexpresolve:SizeMismatch', ...
...
if (~isempty(Acurr)) 
    ...
    if (~restoreBasis) % turn off the forcing constraint case temporarily
...
if isempty(Acurr) && ~isempty(lbcurr)
    % Only the lb anf ub boundary conditions need to be satified:
