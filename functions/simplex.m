...
%   $Revision: 1.1.6.1 $  $Date: 2006/11/11 22:48:00 $
...
if ischar(maxiter)
  if isequal(lower(maxiter),'100*numberofvariables')
    ...
    error('optim:simplex:InvalidMaxIter', ...
          'Option ''MaxIter'' must be an integer value if not the default.')
  end
end

% Final output messages:
presolveTermMsg = 'Optimization terminated during preprocessing phase.';
presolveOptMsg = 'Optimization terminated during preprocessing phase.';
presolveInfsMsg = 'Exiting: The constraints are overly stringent; no feasible point exists.';
unboundedMsg = 'Exiting: The problem is unbounded; the constraints are not restrictive enough.';
excdMaxiterPhase1Msg = sprintf(['Exiting: Maximum number of iterations exceeded in Phase 1;' ...
                          '\n' '         increase options.MaxIter.']);
phaseoneInfsMsg = 'Exiting: The constraints are overly stringent; no feasible starting point found.';
excdMaxiterPhase2Msg = 'Exiting: Maximum number of iterations exceeded; increase options.MaxIter.';
optMsg = 'Optimization terminated.';
...
if exitflagp < 0  
    ...
    elseif exitflag == -2     % This value will be remapped to -3 in linprog.m
      ...
      error('optim:simplex:WrongStatus','Wrong status.')
    end
    if verbosity > 0
      disp(msg)
    end
...
if (exitflagPhase1 == 1) && all(c == 0)
    computeLambda = false;
    if (verbosity >=2)
        disp('The objective coefficients are all zeros or empty, skipping Phase 2.');
    end
end
...
if (nargin < 3), ble = [];
    if (nargin < 2), Ale = [];
        if (nargin < 1) 
            error('optim:simplex:NotEnoughInputs', ...
                  'simplex requires at least one input.');
        end, end, end
...
if (size(c,2) ~= 1)  
    error('optim:simplex:InvalidFirstInput', ...
          'First input to lpdriver must be a column vector of weights.')
end

if any(isnan([c; lb; ub])) || any(isnan([ble;beq])) || any(any(isnan([Ale; Aeq])))
    exitflagDatacheck = -1; 
    error('optim:simplex:NaNInput','Exiting: Input arguments cannot have NaN values.');
end

if  any(any(isinf([c'; Ale; Aeq]))) || any(isinf([ble;beq]))
    exitflagDatacheck = -1;
    error('optim:simplex:InfInput', ...
          ['Exiting: Input arguments (other than the upper or lower bounds) ' ...
           'cannot have Inf values.']);
end
