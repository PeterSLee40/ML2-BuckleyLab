function [eventtimes,events]=CNSvitals_4PH(subjectid,patdate,ext,savefigures,timeaxis,Markstime,timesyncmark_dcs,savematfile,rotatelabel,invivo,hrmin,bpmin,bpmax,spo2min,rapmin,mapnans)
...
if invivo
    ...
    fnametmp=[ fdir 'inCO2,na,Numeric,Float,Solar,data_part1of1.mat' ];
end
if exist(fnametmp)
    load([ fnametmp ]);
...
if invivo
    ...
    fnametmp=[ fdir 'RAP,na,Numeric,Float,Solar,data_part1of1.mat' ];
end
if exist(fnametmp)
    load([ fnametmp ]);
...
if exist(fnametmp)
    ...
    set(gca,'FontSize',34)
    xlim([min(timeaxis) max(timeaxis)])
    ylim([min(nbpdias(find(nbpdias >= 0)))-5 max(nbpsys)+5])
    tmplim3=get(gca,'YLim');
    for kkkk=1:length(eventtimes)
        line([eventtimes(kkkk) eventtimes(kkkk)],[tmplim3(1) tmplim3(2)],'Color',[0 0 0])
        if rotatelabel==0
...
if invivo
    ...
    fnametmp=[ fdir 'ABP_DIAS,na,Numeric,Float,Solar,data_part1of1.mat' ];
end
if exist(fnametmp)
    load([ fnametmp ]);
    ...
    if invivo
        ...
        fnametmp=[ fdir 'ABP_SYST,na,Numeric,Float,Solar,data_part1of1.mat' ];
    end
    load([ fnametmp ]);
    ...
    if ~invivo
        fnametmp=[ fdir 'MAP,na,Numeric,Float,Solar,data_part1of1.mat' ];
        load([ fnametmp ]);
