...
% On input, x is the initial guess, f and grad are the values of the function 
% and the gradient, respectively, both evaluated at the initial guess x. The 
% input variable hessian is the initial quasi-Newton matrix.
% On output, x is the computed solution, f and grad are the values of 
% the function and the gradient, evaluated at the computed solution x. 
% The output variable hessian is the final quasi-Newton matrix.
 
%   Copyright 1990-2007 The MathWorks, Inc.
%   $Revision: 1.1.6.5 $  $Date: 2007/06/14 05:19:06 $
...
[done,exitflag,outMessage] = initialTestStop(g0Norm,TolFun); 
...
while ~done
    ...
    [alpha,f,gPlusTimesDir,grad,exitflagLnSrch,funcCountLnSrch] = ... 
            ...
            eps(max(1,abs(f))),DiffMinChange,DiffMaxChange,TypicalX,varargin{:});
...
elseif exitflagLnSrch == -2
  outMessage = sprintf(['Line search cannot find an acceptable point along the current\n' ...
                        ' search direction.']);
  exitflag = -2;
end

% Display final message - if display is ('final' or 'iter')
% or ('nootify' and exiftlag <= 0)
if verbosity > 1 || verbosity ==  1 && exitflag <= 0
  disp(outMessage);
end
...
% OutputFcn call
if haveoutputfcn || haveplotfcn
  [xOutputfcn, optimValues] = callOutputAndPlotFcns(outputfcn,plotfcns,x,xOutputfcn,'done',iter,funcCount, ...
      f,alpha,grad,dir,varargin{:});      
end   
...
function [done,exitflag,msg] = ...
                        ...
                        TolFun,maxIter,maxFunEvals,grad,g0Norm)
%
...
%-------------------------------------------------------------------------------
function [done,exitflag,msg] = initialTestStop(g0Norm,TolFun)
%
% INITIALTESTSTOP checks if the starting point satisfies a convergence
% criterion.
%
if g0Norm < TolFun
  msg = sprintf(['Optimization terminated at the initial point: the magnitude\n' ... 
                 ' of the gradient at x0 less than options.TolFun.']);     
  done = true;
  exitflag = 1;
else
  exitflag = [];
