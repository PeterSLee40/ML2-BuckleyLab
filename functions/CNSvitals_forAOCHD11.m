function CNSvitals_forAOCHD11(subjectid,patdate,ext,savefigures,timeaxis,Markstime,timesyncmark_dcs,savematfile,rotatelabel,invivo,hrmin,bpmin,bpmax,spo2min,rapmin)

fdir=[ '../' subjectid '/' subjectid 'notes/' subjectid '_' patdate 'vitals/'];
hrlim=[80 200];
bplim=[20 110];
SpO2lim=[50 100];
raplim=[0 20];
...
%2 sync marks with CNS in this data set
ind=find(synctmp);
if findstr(ext,'1')
    sync=synctmp(ind(1));
else
...
if exist(fnametmp)
    ...
    if savematfile
        tt=['save vitals' subjectid '_' patdate ext ' eventtimes sync hr time_hr invivo'];
        eval(tt);
    end
end
clear fnametmp
...
if exist(fnametmp)
    ...
    if ~invivo
        ...
        for i=1:length(map)
            if map(i)<=bpmin || map(i)>=bpmax
                map(i)=NaN;
            end
        end    
    end
    
    figure,plot(time_bpsys,bpsys,'.b-','MarkerSize',30,'LineWidth',3)
    hold on,plot(time_bpdias,bpdias,'.r-','MarkerSize',30,'LineWidth',3)
    if ~invivo
        hold on,plot(time_map,map,'.-','Color',[0 0.5 0],'MarkerSize',30,'LineWidth',3)
    end
    set(gca,'FontSize',34)
    xlim([min(timeaxis) max(timeaxis)])
    ylim(bplim)
