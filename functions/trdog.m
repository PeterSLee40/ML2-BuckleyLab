...
%   $Revision: 1.1.6.1 $  $Date: 2006/11/11 22:48:11 $
...
if n > 1 
   ...
   if isnan(sg)
      % No truncation if s is zero length
      error('optim:trdog:NaNInGrad','Gradient step contains NaN''s.')
   end
   if  isempty(find(arg))   
      alpha = 1;
   else
...
tau = min(1,tau);
if tau <= 0, 
   error('optim:trdog:SqrRt','Square root error in trdog/quad1d.'); 
end
nx = tau*x;
