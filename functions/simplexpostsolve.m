...
%   $Revision: 1.1.6.1 $  $Date: 2006/11/11 22:48:04 $
...
if  diagnostic_level >=4
    ...
    if ( (abs(fval0) > 1.0e-12)  && (abs(fval - fval0)/abs(fval0) > 1.e-6) ) || ... % relative error
        ...
        error('optim:simplexpostsolve:RelErrors', ...
              'Problem with relative errors in presolve and postsolve procedures.');
    end
end
...
if restoreBasis
    ...
    % Case: forcing constraints
    % Temporarily turn off this case in simplexpresolve. This case needs to
