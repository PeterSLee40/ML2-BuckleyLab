...
%GENERATEMfile Generate M-file helper function for
%   optimguiGeneratemfile. This function takes a problem structure 'probStruct'
%   and an options structure 'optStruct' and generates a M-file. The generated
%   code sets the non-default options and also calls the appropriate solver.
%
%   Private to OPTIMTOOL

%   Copyright 2005 The MathWorks, Inc.
%   $Revision: 1.1.6.1 $  $Date: 2006/11/11 22:47:14 $
...
for i = 1:problemNumFields
    ...
    solverCode = [solverCode tempcode sprintf(',')];
        % If value is numeric and a matrix/vector then this code also goes into
        % the headerCode.
        if(length(probValue) > 1) && isnumeric(probValue)
...
optionsCode = [optionsCode sprintf('options = optimset;\n')];
...
for i = 1:optionsNumFields
    ...
    if  ~isempty(optValue)  % don't generate code for defaults.
        ...
        optionsCode = [optionsCode sprintf('options = optimset(options,''%s'' ,%s);\n', ...
            optField,tempcode)];
        % If value is numeric and a matrix/vector then this code also goes into
        % the headerCode.
        if(length(optValue) > 1) && isnumeric(optValue)
            headerCode = [headerCode tempcode sprintf(',')];    
        end
    end
end
...
if(length(value) > 1) && isnumeric(value)
    if problem_type
        rhsValue = sprintf('%s',FieldName); % This is for problem values that are matrices/vectors
    else
