...
%		errors = errors';
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
		handles.legend = legend(bw_legend, 'location', 'best', 'fontsize',18);
		legend boxoff;
	else
		handles.legend = [];
	end
	...
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			...
			for j = 1:length(xdata)
				text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 60, 'fontsize', 18, 'HorizontalAlignment', 'right');
			end
		end
	...
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', 18, 'xtick',1:numgroups, 'linewidth', 1,'xgrid','off','ygrid','off');
