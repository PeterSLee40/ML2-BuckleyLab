...
plot(x,y,linecol,'LineStyle','none')
hold on
...
for t=1:length(x)
    ...
    if ~isempty(ux)
        %x errorbars
        line([x(t)-lx(t) x(t)+ux(t)],[y(t) y(t)],'color',errorcol,'LineWidth',3)
        line([x(t)-lx(t) x(t)-lx(t)],[y(t)-yw y(t)+yw],'color',errorcol,'LineWidth',3)
        line([x(t)+ux(t) x(t)+ux(t)],[y(t)-yw y(t)+yw],'color',errorcol,'LineWidth',3)
    end
    
    if ~isempty(uy)
        %y errorbars
        line([x(t) x(t)],[y(t)-ly(t) y(t)+uy(t)],'color',errorcol,'LineWidth',3)
        line([x(t)-xw x(t)+xw],[y(t)-ly(t) y(t)-ly(t)],'color',errorcol,'LineWidth',3)
        line([x(t)-xw x(t)+xw],[y(t)+uy(t) y(t)+uy(t)],'color',errorcol,'LineWidth',3)
    end
end
hold off
