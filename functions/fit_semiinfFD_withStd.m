function f=fit_semiinfFD_withStd(x0,n,Reff,lambda,r,w,AC,ph,ACstd,phstd)

mua=x0(1);
musp=x0(2);

for j=1:length(r)
    [amp(j),phase(j)]=diffusionforwardsolver(n,Reff,mua,musp,0,0,lambda,r(j),w,0);
end
%{
ind=find(r==min(r));

%Normalize amp and phase to smallest s-d separation
amp=amp./amp(ind);
phase=phase-phase(ind);
AC=AC./AC(ind);
ph=ph-ph(ind);

indamp=find(amp~=1);
indph=find(phase~=0);
%}
%f=norm(cat(2,((log(amp(indamp))-log(AC(indamp)))./log(amp(indamp))).^2 , ((phase(indph)-ph(indph))./phase(indph)).^2));
%f=norm(cat(2,(log(amp(indamp))-log(AC(indamp)))./log(amp(indamp)) , (phase(indph)-ph(indph))./phase(indph)));
%f=norm(cat(2,(log(AC)-log(amp))./log(AC) , (ph-phase)));
%f=norm(cat(2,(amp-AC)./amp , (phase-ph)./phase));
%f=norm(cat(2,(log(AC)-log(amp))./log(ACstd) , (ph-phase)./phstd));

%Try weighting phase such that the range the phase spans is of equal
%magnitude to the range the amplitude spans
ACrange = max(log(AC)) - min(log(AC));
phrange = max(ph) - min(ph);

f=norm(cat(2,(log(AC)-log(amp)), (ph-phase)*ACrange/phrange ));
