...
% end
% 
inputtarget = single([input inputnn target]);
inputtarget = single(inputtarget(randperm(size(inputtarget,1)),:));
inputshuffle1 = inputtarget(:, 1:size(tau,2));
inputshuffle2 = inputtarget(:, size(tau,2) + 1: 2*size(tau,2));
inputnnshuffle = inputtarget(:, 1 + size(input,2):2*size(input,2));
targetshuffle = inputtarget(:, size(input,2)*2 + 1);
%inputflip = fliplr(inputshuffle);


csvwrite('TSG_db2_2_target.csv',targetshuffle)
csvwrite('TSG_db2_2_input.csv',[inputshuffle1, inputshuffle2])
csvwrite('TSG_db2_2_inputnn.csv',inputnnshuffle)

nnstart

% csvwrite('TSG_db2_2_input1.csv',inputshuffle1)
% csvwrite('TSG_db2_2_inputnn1.csv',inputnnshuffle1)
% csvwrite('TSG_db2_2_input2.csv',inputshuffle2)
% csvwrite('TSG_db2_2_inputn2.csv',inputnnshuffle2)

%run TSG_db2_2, then run autoencoder2, then run autoencoder2eval, then run
%TSG_db2_2_denoise_and_fit