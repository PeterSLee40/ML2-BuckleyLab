function phi = diffusionforwardsolver1(n,Reff,mua1,mus1,aDb1,tau,lambda,rho,w,ell,mua2,mus2,aDb2)
%
%amp is the calculated amplitude of the fluence rate (DOS) or 
%    the normalized electric field temporal correlation function (DCS) 
%
...
%          single layer.
%
%cutoff specifies how many weights in Gauss-Laguerre Quadrature to to take
%for doing the integral.  I usually find 500 is good.
...
% phit1=(sinh(alpha1*(zb + z0)) ./ (D1*alpha1)) .* ((D1*alpha1.*cosh(alpha1*ell) + D2*alpha2.*sinh(alpha1*ell)) ...
%          ./ (D1*alpha1.*cosh(alpha1*(ell+zb)) + D2*alpha2.*sinh(alpha1*(ell+zb))));
% 
%  phit2=-sinh(alpha1*z0)./(D1*alpha1);
%  phit=phit1+phit2;
%      phit = ((exp(alpha1*(zb + z0))/2-exp(-alpha1*(zb + z0))/2) ./ (D1*alpha1)) .* ((D1*alpha1.*(exp(alpha1*ell)/2+exp(-alpha1*ell)/2) + D2*alpha2.*(exp(alpha1*ell)/2-exp(-alpha1*ell)/2)) ...
%          ./ (D1*alpha1.*(exp(alpha1*(ell+zb))/2+exp(-alpha1*(ell+zb))/2) + D2*alpha2.*(exp(alpha1*(ell+zb))/2-exp(-alpha1*(ell+zb))/2))) - (exp(alpha1*z0)/2-exp(-alpha1*z0)/2)./(D1*alpha1);
%     
   
      
   invphit = lagw .* phit .* S .* besselj(0,S*rho);
      % invphit = lagw .* phit  .* besselj(0,S*rho);
