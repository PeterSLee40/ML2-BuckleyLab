...
Beta = 0.35:.05:.55;
Intensity = 20e3;
siz = size(Db1s,2)*size(Ratio1,2)*size(ells,2)*size(Rep,2)*size(Intensity,2);
input = zeros(siz,200);
target = zeros(siz,3);
beta = .5;
    ...
    for ratio = Ratio1
        for l = ells
            ...
            for intensity = Intensity
                ...
                gamma = 1/tau(index25);
                sigma25 = getDCSNoise(intensity,T,3,beta,gamma,tau);
                for rep = Rep
                    j = j+1;
                    if noise == true
                        for i = 1:7
...
inputbeta = zeros(siz*size(Beta,2),200);
