function [amp phase] = diffusionforwardsolverglc(n,Reff,mua1,mus1,aDb1,tau,lambda,rho,w,ell,mua2,mus2,aDb2, gl)
...
if mua2 == 0
    ...
    lagw = lagw .* exp(S);
    sizet = size(tau,2);
    alpha1sq = (repmat(D1*S.^2,1,sizet) + repmat(mua1,500,sizet) + repmat(2*mus1*k0^2*tau*aDb1 ,500,1)+ repmat(1i*w/v, 500, sizet)) ./ D1;
    alpha1 = sqrt(alpha1sq);
    alpha2sq = (repmat(D2*S.^2,1,sizet) + repmat(mua2,500,sizet) + repmat(2*mus2*k0^2*tau*aDb2 ,500,1) + repmat(1i*w/v,500, sizet)) ./ D2;
    ...
    invphit = repmat(lagw, 1, sizet) .* phit .* repmat(S, 1, sizet) .* repmat(besselj(0,S*rho), 1, sizet);
