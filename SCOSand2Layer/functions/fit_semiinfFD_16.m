function f=fit_semiinfFD_16(x0,n,Reff,lambda,r,w,AC,ph)
%Method 16
mua=x0(1);
musp=x0(2);
...
%Adjust AC and ph for source strength
AC=AC*amp(ind)/AC(ind);
ph=ph+phase(ind)-ph(ind);
...
%f=norm(cat(2,(log(AC)-log(amp))./log(AC) , (ph-phase)./ph));
...
ACrange = max(log(AC)-log(amp.')) - min(log(AC)-log(amp.'));
phrange = max(ph-phase.') - min(ph-phase.');
f=norm(cat(2,log(AC)-log(amp.'), (ph-phase.')*ACrange/phrange ));
