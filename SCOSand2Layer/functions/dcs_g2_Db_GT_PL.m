function result = dcs_g2_Db_GT_PL(x,tau,g2,p,mu_a,mu_sp,alpha,k0,Reff)
...
msd = 6*Db * tau ;
Kappa = sqrt(3*mu_sp*mu_a + mu_sp^2 * k0^2 * alpha.* msd);
taurep1 = repmat(r1,1,size(tau,2));
taurep2 = repmat(r2,1,size(tau,2));
G1 = exp(-Kappa .* taurep1)./taurep1 - exp(-Kappa .* taurep2)./taurep2;
