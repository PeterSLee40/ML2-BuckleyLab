...
%   $Revision: 1.1.6.2 $  $Date: 2006/12/15 19:29:09 $
...
% Need to evaluate the value if not knwon.
% If 'type' is 'string' then it can not be evaluated in MATLAB base (hence try-catch).
% If 'type' is 'mixed' then the GUI returns the value as [] so we don't need to evaluate.
% If 'type' is 'other' then it is numeric and can be safely evaluated in MATLAB base.
if strcmp(value,'[]') || ~strcmp(type,'string')
    try
        value = evalin('base', value);
    catch % Check if the value is already in the structure (encrypted form)
...
stringOnly =   {'Simplex','NonlEqnAlgorithm', ...
    ...
    'Hessian','GradObj', ...
    'GradConstr','Diagnostics', ...
    'DerivativeCheck','BranchStrategy', ...
    'FunValCheck','Display','solver'};
...
stringAndOther = {'TypicalX','MaxSQPIter', ...
...
if ismember(optName,stringOnly)
    type = 'string';
elseif ismember(optName,stringAndOther)
    type = 'mixed';
else 
    type = 'other';
end
