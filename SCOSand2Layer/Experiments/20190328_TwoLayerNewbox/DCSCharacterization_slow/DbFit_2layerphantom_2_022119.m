...
usedflowdets=[3];%must be consecutive, cannot be ex. 1,3,4 or 2,4.  must change code if this is the case
SD_dist=[15];%mm
...
file_list = dir([ fdir  'slow_10mm_*_flow*.dat']);
...
for d = 1:length(usedflowdets)
    meang2(d,:) = nanmean(corr(3,usedflowdets(d),:),1);
end
...
hold on;
fit = dcs_g2fit_GT([nanmean(Dbfit(3)), nanmean(beta(3))],taus,SD_dist,muao,muspo,k0,R,1)
