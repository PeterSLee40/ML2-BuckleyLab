...
lambdaDCS=850;%wavelength in nm
...
file_list = dir([ fdir  'trial3*flow*.dat']);
...
for d = 1:length(usedflowdets)
    meang2(d,:) = nanmean(corr(2:end - 1,usedflowdets(d),:),1);
end
figure,semilogx(taus,meang2);
hold on;
semilogx(taus,squeeze(nanmean(Curvefitg2avg(2:end - 1,:,:), 1)),'k--','LineWidth',2)
