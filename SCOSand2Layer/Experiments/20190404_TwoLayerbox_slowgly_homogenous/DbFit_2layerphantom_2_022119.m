...
usedflowdets=[3, 4, 5, 6];%must be consecutive, cannot be ex. 1,3,4 or 2,4.  must change code if this is the case
...
cutoff=1.05;%where to cut correlation curve off when fitting
...
file_list = dir([ fdir  'trail1_flow*.dat']);
...
for d = 1:length(usedflowdets)
    meang2(d,:) = nanmean(corr(2,usedflowdets(d),:),1);
end

index = 2;
figure,semilogx(taus(1:100),meang2(:,1:100))
hold on;
fit = squeeze(Curvefitg2avg(index, :, :));
semilogx(taus(1:100),fit(:, 1:100));
hold on
ylim([0.9 1.6])
xlim([min(taus) 1e-2])
save('meandata.mat','meang2','taus')
legend('10mm-exp', '15mm-exp', '20mm-exp', '25mm-exp', ...
'10mm-fit', '15mm-fit', '20mm-fit', '25mm-fit')
