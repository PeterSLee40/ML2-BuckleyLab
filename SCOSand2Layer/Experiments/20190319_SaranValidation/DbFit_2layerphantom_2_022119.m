...
usedflowdets=[5 7 8];%must be consecutive, cannot be ex. 1,3,4 or 2,4.  must change code if this is the case
SD_dist=[10 20 25];%mm
...
file_list = dir([ fdir  'Saran_*_flow_*.dat']);
...
for d = 1:length(usedflowdets)
    ...
    plottedfit(d,:) = dcs_g2fit_GT([nanmean(Dbfit(:,d)), nanmean(beta(:,d))],taus,SD_dist(d),muao,muspo,k0,R,1)';
end
figure,semilogx(taus(5:100),meang2(:,5:100));
hold on;
semilogx(taus(5:100),plottedfit(:, 5:100 ));
