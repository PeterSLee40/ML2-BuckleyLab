...
Db1s = [.90*db1prediction: .01*db1prediction: 1.01*db1prediction];
...
multitau = repmat(tau,size(Rhos,2),1);
    ...
    for ratio = Ratio
        for l = ell
            for rep = 1:Rep
                ...
                for i = 1:size(Rhos,2)
                    rho = Rhos(i);
                    [g1s(i,:) gamma(i,:)]= getG1(n,Reff,curmua1,curmus1,db1,tau,lambda,rho,w,l*(1+randn()*.025),curmua2,curmus2,db2,gl);
                end
                sg1s = squeeze(g1s)';
                sgamma = squeeze(gamma)';
                for beta = 1:Betas,    j = j + 1;
                    %betaRand = meanBeta.*(randn(1).*meanbetastdfit.*2+1);
                    betaRand = meanBeta;
                    sigmas = getDCSNoise(intensities, T, inttime, betaRand, sgamma, multitau);
                    noises = sigmas.*randn(numDetectors, size(tau,2));
                    %betasRand = repmat(betaRand',1,size(tau,2));
                    g2s = betaRand'.*sg1s.^2 + 1;
